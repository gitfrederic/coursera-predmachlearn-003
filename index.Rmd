...
```{r chunk2, echo=TRUE, message=FALSE}
...
```{r chunk3, message=FALSE, cache=TRUE, echo=TRUE}
...
fit <- train(classe ~ ., data=trainingData, method="rf", ntree=5, trControl=fitControl)

```

<p/>
The *ntree* parameter was initially set higher, but for the purpose of this exercise (to predict the testing dataset values) an ntree of '5' gives the same answers as higher values of ntree (and it takes much less time to process).  In the next section, I discuss the *oob* sampling/cross-validation method set in the trainControl.
...
```{r chunk4,echo=TRUE,message=FALSE}
